    ...
    const char *stmt_text, char *err, unsigned err_size)
{
    ...
    for (i = 0; i < arg_count; ++i) {
        if (arg_types[i][0] != '\0' && callee->arg_types[i][0] != '\0'
                && strcmp(arg_types[i], callee->arg_types[i]) != 0) {
    ...
    for (i = 0; i < target_count; ++i) {
        if (target_types[i][0] != '\0' && callee->result_types[i][0] != '\0'
                && strcmp(target_types[i], callee->result_types[i]) != 0) {
...
static int plc_next_row_word(const char **p, char *out, unsigned out_size)
{
    unsigned n;
...
static int plc_expand_2d_row(const char *expr_row, const char *index_row,
    ...
    char *err, unsigned err_size)
{
    ...
    while (*expr != '\0') {
        if ((*expr == 'V' || *expr == 'Z' || *expr == 'R')
                && !isalnum((unsigned char)expr[1])) {
            char index[32];
            char type[PLC_MAX_TYPE_TEXT];
            char ref[96];

            if (!plc_next_row_word(&idx, index, sizeof(index))
                    || !plc_next_row_word(&typ, type, sizeof(type))) {
                plc_set_error(err, err_size, "bad two-dimensional row");
                return 0;
            }
            sprintf(ref, "%c%s[:%s]", *expr, index, type);
        ...
        while (fgets(line, sizeof(line), fp) != 0) {
            ...
            if (in_proc && line[0] == '|') {
                ...
                if (fgets(vrow, sizeof(vrow), fp) != 0) {
                    ...
                    if (strncmp(vrow, "V|", 2) == 0
                            && fgets(srow, sizeof(srow), fp) != 0) {
                        ++line_no;
                        plc_strip_comment(srow);
                        if (strncmp(srow, "S|", 2) == 0) {
                            if (!plc_expand_2d_row(expr_copy, vrow + 2,
                            ...
                            sprintf(err, "%s:%d expected S| row",
                                sources[file_index], line_no);
                            fclose(fp);
                            return 0;
                        }
