...
| 8086/DOS ASM backend | supported profile | `plankac asm8086 output.asm` emits MASM/TASM-style 16-bit source with bytecode image, DOS start/smoke entry, direct arithmetic procedures, compound value heap declarations, a compound procedure table, and documented 16-bit ABI entries for lists, sets, pairs, records, relations, predicates, boards, and compound dispatch |
...
| C embedding API | supported | `PLANKAC_CONTEXT`, custom source lists, bytecode loading, procedure metadata, run by name or number, typed result values, IR/bytecode/C/ASM/8086 output |
| Conformance checks | supported | valid and invalid `.plk` fixtures under `tests/conformance` |
| Two-dimensional original notation | supported profile | executable `|` statement rows with aligned `V|`/`S|` tables are accepted, including swapped row order, spaced cell alignment, symbolic cells, multi-table and block-separated `PAGE`/`ENDPAGE` documents, coordinate-bound row selection, and row/column diagnostics |
...
| Lists and pairs | supported profile | shared frame-heap list handles plus pair helpers, list equality, generalized predicate syntax (`SELECT`, `COUNT`, `EXISTS`, `FORALL` with `=`, `!=`, `<`, `<=`, `>`, `>=`), zipped pair lists, and list-pair handles |
...
| Type/value model | supported profile | markers are parsed into families with width/scale pieces, checked across calls, checked by structural schema inference for lists, sets, relation domain/range pairs and nested record fields, and backed by active tagged `PLC_VALUE` scalar bank slots plus tagged entries for indexed and field values |
...
and DOS-oriented 8086 ASM source emission.
