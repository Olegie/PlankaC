...
| Two-dimensional original notation | supported profile | executable `|` statement rows with aligned `V|`/`S|` tables are accepted, including swapped row order, spaced cell alignment, symbolic cells, multi-table `PAGE`/`ENDPAGE` documents, coordinate-bound row selection, and row/column diagnostics |
