...
| Arithmetic expressions | supported | `+`, `-`, `*`, `/`, `%`, `^`, parentheses; scalar runtime evaluates the bounded expression AST first and keeps the legacy text parser as compatibility fallback |
...
| Typed IR | supported | source loading builds and validates a typed statement IR with expression tree shape; `plankac ir output.ir` emits a readable IR view |
...
| 8086/DOS ASM backend | supported profile | `plankac asm8086 output.asm` emits MASM/TASM/Open Watcom-oriented 16-bit source with bytecode image, DOS start entry, direct arithmetic procedures, compound value heap declarations, a compound procedure table, and documented 16-bit ABI entries for lists, sets, pairs, records, relations, predicates, boards, and procedure-number compound dispatch |
| DOS COM binary backend | supported bootstrap profile | `plankac doscom output.com` emits a small 8086 `.COM` image directly from `c/backends/dos`, without an external assembler; the image carries a DOS interrupt print path, target marker, procedure count, and embedded `PLANKAC-BYTECODE 0.1` payload |
| PlankaC DOS runner | supported target route | `build-dos-plankac.bat` builds `PLANKACD.EXE` with Open Watcom; the same API target is compiled as `plankacd_host.exe` and checked with `check`, `run`, `tests`, `bytecode`, `checkbc`, and `runbc` |
| Compiler pipeline | supported | `plankac compile prefix` writes `.pbc`, reloads IR, then emits C, x86-64 ASM, 8086 ASM, and DOS COM bootstrap output; `native-c` and `native-asm` link executables |
| Backend lowering report | supported | `plankac lowering output.txt` classifies typed IR statements for direct C lowering, native ASM, 8086 integer core, and list/set/pair/record/relation/predicate/chess/geometry/complex runtime entry points |
| C embedding API | supported | `PLANKAC_CONTEXT`, custom source lists, bytecode loading, procedure metadata, run by name or number, typed result values, IR/bytecode/C/ASM/8086/DOS COM output |
| Conformance checks | supported | valid and invalid `.plk` fixtures under `tests/conformance` |
| Two-dimensional original notation | supported profile | executable `|` statement rows with aligned `V|`/`S|` tables are accepted, including swapped row order, spaced cell alignment, symbolic cells, multi-table and block-separated `PAGE`/`ENDPAGE` documents, explicit `[row,column]` page coordinates, model-based symbol-to-cell expansion, coordinate-bound row selection, and row/column diagnostics |
...
| Lists and pairs | supported profile | shared frame-heap list handles plus pair helpers, list equality, generalized predicate syntax (`SELECT`, `COUNT`, `EXISTS`, `FORALL`, `NOT <predicate>` with `=`, `!=`, `<`, `<=`, `>`, `>=`), zipped pair lists, and list-pair handles |
| Sets / relations | supported profile | `set_new`, `set_add`, `set_contains`, `set_union`, `set_intersection`, `set_difference`, `set_subset`, `set_equal`, `set_cartesian`, `relation_inverse`, `relation_image`, `relation_compose`, `relation_select_domain`, `relation_select_range`, `SET*`, `DOMAIN*`, and `RANGE*` predicate syntax, relation domain/range/pair predicates, quantifier helpers, and deterministic relation signatures |
...
| Type/value model | supported profile | markers are parsed into families with width/scale pieces, checked across calls, checked by structural schema inference for lists, sets, relation domain/range pairs and nested record fields, and backed by active tagged `PLC_VALUE` scalar bank slots, typed procedure argument/result boundaries, plus tagged entries for indexed and field values |
...
compound value handles, chess-board modeling, expression-tree AST artifacts,
typed IR, bytecode, lowering classification, generated C, native x86-64 ASM,
DOS-oriented 8086 ASM source emission, built-in DOS COM bootstrap emission,
and the PlankaC DOS runner target route.
