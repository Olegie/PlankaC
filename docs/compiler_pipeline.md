...
.plk source
    ...
    -> typed IR / bytecode
    -> IR reload
    -> generated C / generated x86-64 ASM / 8086 ASM source / DOS COM bootstrap
...
The lowering report includes AST node counts, call/predicate counters, and the
serialized expression-tree shape that drove the backend decision.
...
doscom: build\plankac_pipeline_dos.com
```
...
| 8086 backend | `_8086.asm` | MASM/TASM/Open Watcom-oriented 16-bit source with bytecode image, DOS start entry, direct arithmetic procedures, compound heaps, compound procedure table, and compound dispatch ABI |
| DOS COM backend | `_dos.com` | built-in 8086 bootstrap and embedded bytecode container emitted directly by PlankaC without MASM/TASM/Open Watcom |
| DOS runner | `PLANKACD.EXE` | real 16-bit DOS MZ runner built with Open Watcom from the PlankaC API target |
...
plankac.exe doscom build\plankac_dos.com
plankacd_host.exe check
plankacd_host.exe run add 12 8
plankacd_host.exe tests
plankacd_host.exe bytecode build\plankacd_host.pbc
plankacd_host.exe checkbc build\plankacd_host.pbc
plankacd_host.exe runbc build\plankacd_host.pbc set_session
```

That keeps the compiler path part of normal verification instead of a
separate manual demonstration.

For the generated 8086 path, `build-asm8086-dos.bat` emits
`build\dos\PLANKAC86.ASM` through PlankaC and assembles it into a DOS `.EXE`
when MASM/TASM with LINK/TLINK or Open Watcom `wasm` with `wlink` are
available.

For a full DOS executable, `build-dos-plankac.bat` builds `PLANKACD.EXE` with
Open Watcom. The same source file is compiled as `plankacd_host.exe` by
`build.bat`, so the runner commands are verified even when the 16-bit DOS
toolchain is not installed on the current machine.

For a toolchain-free DOS artifact, `plankac.exe doscom output.com` emits a
small `.COM` bootstrap directly from PlankaC and appends the generated bytecode
image to that binary. The bootstrap stays inside the 64K `.COM` model; full
execution is handled by `PLANKACD.EXE`.
